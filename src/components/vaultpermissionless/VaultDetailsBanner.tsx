'use client'
import { InfoIcon, NextIcon } from "@/components/common/Icons";
...
const VaultDetailsBanner = ({ params }: Params) => {
  const chartContainerRef = useRef<HTMLDivElement>(null);
  const chartInstanceRef = useRef(null);
  const [legendText, setLegendText] = useState('ETC USD 7D VWAP');
  ...
  useEffect(() => {
    ...
    const chart = createChart(chartContainerRef.current, { 
      ...
      rightPriceScale: {
        ...
        crosshair: {
          vertLine: { width: 1, color: 'rgba(224, 227, 235, 0.1)', style: 0 },
    ...
    <div className="w-full ">
      ...
      <div className="container relative z-10 xl:max-w-[1140px] w-full px-4 mx-auto sm:pt-9 pb-10    text-white">
        <div className=" flex justify-between items-center">
          ...
          <div className="legend" style={{ color: 'white' }}>{legendText}</div>
          <div ref={chartContainerRef} className="chart-container" style={{ width: '100%' }} />
        </div>
        </div>
      </div>
    </div>
